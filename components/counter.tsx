...
export function StatsBar(options: Options) {
    return (
        <Box
            none
            className="flex w-full rounded-md overflow-hidden divide-x divide-wamellow"
        >
            {options.items.map((item, i) => (
                <div className={cn("p-5 dark:bg-wamellow bg-wamellow-100 flex-1 min-w-0", i >= 2 && "hidden md:block")} key={"counter" + item.name}>

                    <div className="flex">
                        <div className="text-sm font-medium mb-1">{item.name}</div>
                        {item.info && (
                            <Link href={item.info} className="ml-auto dark:text-neutral-400 text-neutral-600 dark:hover:text-violet-400 hover:text-violet-600 duration-300">
                                <HiInformationCircle />
